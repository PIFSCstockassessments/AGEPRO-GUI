...
* No Stock of Age, but Output Auxilary Files EXCEPT Stock Numbers Of Age 
...
* Output Stock of Age and ALL Auxilary Files                             
  * This Will Output Stock Numbers at Age and Auxiliary files. This is compatible with
    `AGEPRO VERSION 4.0` and `AGEPRO VERSION 4.25` input files that have the option.  
* NO Stock of Age and Auxilary Files
  * No Stock Numbers at Age summary report or Auxiliary files will be created. This is only compatible with `AGEPRO VERSION 4.25` Input File format.
* Output Stock of Age, but NO Auxilary Files
  * This option produces a population size at age summary in the output file, but does not generate the auxiliary files. 
    This is only compatible with `AGEPRO VERSION 4.25` Input File format. 
* Output Stock of Age and Auxilary Files, EXCEPT Stock Numbers Of Age 
  * This option produces a population size at age summary in the output file, generates the auxiliary files, 
    but does not produce a population size at age file summary. This is only compatible with `AGEPRO VERSION 4.25` Input File format.

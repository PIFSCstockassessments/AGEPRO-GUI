...
* [AGEPRO Model Output Run Directory](#agepro-model-output-run-directory)
	* [Output Directory Naming Format](#directory-naming-format)
...
Launching an AGEPRO model, via the **Launch AGEPRO model** option under **Run** menu, creates a new **AGEPRO Model Output Run directory**. 

Output files generated by the calcuation engine run will be stored here.

In addition, the control's inputs and bootstrap settings are stored here. AGEPRO will check if the form inputs are valid. If valid, it will grab a copy of the bootstrap file, and stores the GUI input under a desginagted **AGEPRO Model Output Run directory** under the `AGEPRO` subdirectory, under the desginagted user document directory (`documents` or `My Documents`). The `AGEPRO` subdirectory will be created if it didn't exist under the document directory.

After the calcuation engine is finished, AGEPRO will open a new window at the **AGEPRO Model Output Run directory** the outputs were written to.

## Directory Naming Format

The **AGEPRO Model Output Run directory** may be entitled two ways:

- It may take base name given in input file from general options (without file extensions) appended with a date-time stamp when the model was launched. 
- If input file in general options is blank, then it will generate a random filename (without file extensions) and the date-time stamp this model launched.
